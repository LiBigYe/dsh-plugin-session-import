...
function extractToolArguments(p) {
  ...
  try {
    const jsonText = objText.replace(/`/g, '"').replace(/'([^'\\]*(?:\\.[^'\\]*)*)'/g, '"$1"').replace(/([{,]\s*)([A-Za-z_$][\w$]*)\s*:/g, '$1"$2":').replace(/,\s*}/g, "}");
...
async function importOne(ctx, tool2, path, agentOptions, hintCwd, preset) {
...
function apply(ctx) {
  ...
  ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        let rows = [];
        try {
          rows = await ctx.agentPresets?.list?.() ?? [];
        } catch (e) {
          rows = [];
        }
        let defaultId = "";
  ...
  ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        const body = await readBody(req);
        const { tool: tool2, paths, preset } = body ?? {};
        ...
        for (const path of paths) {
          try {
            const r = await importOne(ctx, tool2, path, {}, cwdHint.get(path), preset);
  ...
  ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        const body = await readBody(req);
        const { tool: tool2, path, preset } = body ?? {};
        if (!tool2 || !path) {
          res.writeHead(400, { "content-type": "application/json" });
          res.end(JSON.stringify({ ok: false, error: "\u7F3A\u5C11 tool \u6216 path" }));
          return;
        }
        const r = await importOne(ctx, tool2, path, {}, void 0, preset);
        res.writeHead(200, { "content-type": "application/json" });
